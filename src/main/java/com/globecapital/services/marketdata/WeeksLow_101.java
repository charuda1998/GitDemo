...
public class WeeksLow_101 extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		JSONObject weeksLow = new JSONObject();
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY))
			weeksLow = MarketMovers_101.getWeeksLowEquity(exchange, gcRequest.getAppID(), indexName);

		JSONArray topArr = weeksLow.getJSONArray(DeviceConstants.MARKET_DATA);
		if (topArr.length() != 0) {
			gcResponse.setData(weeksLow);
		} else {
