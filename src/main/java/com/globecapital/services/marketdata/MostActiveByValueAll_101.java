...
public class MostActiveByValueAll_101 extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if(segment.equalsIgnoreCase(DeviceConstants.EQUITY)) {
			...
			mostActiveByValue = MarketMovers_101.getMostActiveByValueEquity(exchange, gcRequest.getAppID(), indexName);
			...
			mostActiveByValue = MarketMovers_101.getMostActiveByValueDerivatives(ExchangeSegment.NFO, instrument, gcRequest.getAppID(), expiry);
			 ...
			 if(expiry.isEmpty() || expiry.equalsIgnoreCase(DeviceConstants.ALL_EXPIRIES))
				...
				expiry = DateUtils.formatDate(expiry, DeviceConstants.ADVANCE_QUOTE_DATE_FORMAT, DeviceConstants.FROM_DATE_FORMAT);
			 instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			 mostActiveByValue = MarketMovers_101.getMostActiveByValueCurrency(exchange, instrument, gcRequest.getAppID(), expiry);
			 ...
			 if(expiry.isEmpty() || expiry.equalsIgnoreCase(DeviceConstants.ALL_EXPIRIES))
				...
				expiry = DateUtils.formatDate(expiry, DeviceConstants.ADVANCE_QUOTE_DATE_FORMAT, DeviceConstants.FROM_DATE_FORMAT);
			 instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			mostActiveByValue = MarketMovers_101.getMostActiveByValueCommodity(ExchangeSegment.MCX, instrument, gcRequest.getAppID(), expiry);
		 }
