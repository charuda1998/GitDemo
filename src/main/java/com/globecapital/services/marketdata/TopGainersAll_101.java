...
public class TopGainersAll_101 extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY)) {
			...
			topGainers = MarketMovers_101.getTopGainersEquity(exchange, gcRequest.getAppID(), indexName);
			...
			topGainers = MarketMovers_101.getTopGainersDerivatives(ExchangeSegment.NFO, instrument, gcRequest.getAppID(), expiry);
			...
			if(expiry.isEmpty() || expiry.equalsIgnoreCase(DeviceConstants.ALL_EXPIRIES))
				...
				expiry = DateUtils.formatDate(expiry, DeviceConstants.ADVANCE_QUOTE_DATE_FORMAT, DeviceConstants.FROM_DATE_FORMAT);
			instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			topGainers = MarketMovers_101.getTopGainersCurrency(exchange, instrument, gcRequest.getAppID(), expiry);
			...
			if(expiry.isEmpty() || expiry.equalsIgnoreCase(DeviceConstants.ALL_EXPIRIES))
				...
				expiry = DateUtils.formatDate(expiry, DeviceConstants.ADVANCE_QUOTE_DATE_FORMAT, DeviceConstants.FROM_DATE_FORMAT);
			instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			topGainers = MarketMovers_101.getTopGainersCommodity(ExchangeSegment.MCX, instrument, gcRequest.getAppID(), expiry);
		}
