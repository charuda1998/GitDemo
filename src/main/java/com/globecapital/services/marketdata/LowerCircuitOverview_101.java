...
public class LowerCircuitOverview_101 extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY))
			lowerCircuit = MarketMovers_101.getLowerCircuitEquity(gcRequest.getAppID());

		if (lowerCircuit.length() != 0) {
			int limit = AppConfig.getIntValue("market_limit");
			JSONArray lowerCircuitOverview = new JSONArray();
			for (int i = 0; i < limit && i < lowerCircuit.length(); i++) {
				lowerCircuitOverview.put(lowerCircuit.get(i));
		}
			gcResponse.addToData(DeviceConstants.MARKET_DATA, lowerCircuitOverview);
		} else {
