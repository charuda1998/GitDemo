...
public class PriceShockers_101 extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		JSONObject priceShockers = new JSONObject();
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY))
			priceShockers = MarketMovers_101.getPriceShockersEquity(exchange, gcRequest.getAppID(), indexName);

		JSONArray topArr = priceShockers.getJSONArray(DeviceConstants.MARKET_DATA);
		if (topArr.length() != 0) {
			gcResponse.setData(priceShockers);
		} else {
