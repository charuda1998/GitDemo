...
public class LowerCircuit_101 extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		JSONObject lowerCircuit = new JSONObject();
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY))
			lowerCircuit = MarketMovers_101.getLowerCircuitEquity(exchange, gcRequest.getAppID(), indexName);

		JSONArray topArr = lowerCircuit.getJSONArray(DeviceConstants.MARKET_DATA);
		if (topArr.length() != 0) {
			gcResponse.setData(lowerCircuit);
		} else {
