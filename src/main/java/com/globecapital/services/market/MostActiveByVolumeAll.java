...
public class MostActiveByVolumeAll extends SessionService {
	...
	protected void doPostProcess(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY)) {
			...
			mostActiveByVolume = MarketMovers.getMostActiveByVolumeEquity(exchange, session.getAppID());
		} else if (segment.equalsIgnoreCase(DeviceConstants.DERIVATIVE)) {
			instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			mostActiveByVolume = MarketMovers.getMostActiveByVolumeDerivatives(ExchangeSegment.NFO, instrument, session.getAppID());
		} else if (segment.equalsIgnoreCase(DeviceConstants.CURRENCY)) {
			exchange = gcRequest.getFromData(DeviceConstants.EXCHANGE);
			instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			mostActiveByVolume = MarketMovers.getMostActiveByVolumeCurrency(exchange, instrument, session.getAppID());
		} else if (segment.equalsIgnoreCase(DeviceConstants.COMMODITY)) {
			instrument = gcRequest.getFromData(DeviceConstants.INSTRUMENT);
			mostActiveByVolume = MarketMovers.getMostActiveByVolumeCommodity(ExchangeSegment.MCX, instrument, session.getAppID());
		}
