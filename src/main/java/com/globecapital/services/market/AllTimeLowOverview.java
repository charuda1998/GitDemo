...
public class AllTimeLowOverview extends SessionService {
	...
	protected void doPostProcess(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY))
			allTimeLow = MarketMovers.getAllTimeLowEquity(session.getAppID());

		if(allTimeLow.length() != 0) {
			gcResponse.addToData(DeviceConstants.MARKET_DATA, allTimeLow);
		} else {
