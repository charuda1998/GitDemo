...
public class WeeksHighOverview extends SessionService {
	...
	protected void doPostProcess(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if (segment.equalsIgnoreCase(DeviceConstants.EQUITY))
			weeksHigh = MarketMovers.getWeeksHighEquity(session.getAppID());

		if (weeksHigh.length() != 0) {
			gcResponse.addToData(DeviceConstants.MARKET_DATA, weeksHigh);
		}	else {
