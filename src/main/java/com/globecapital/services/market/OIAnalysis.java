...
public class OIAnalysis extends SessionService {
	...
	protected void doPostProcess(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if(segment.equalsIgnoreCase(DeviceConstants.DERIVATIVE))
			oiAnalysis = Market.getOiAnalysisDerivatives(category, session.getAppID());
		else if(segment.equalsIgnoreCase(DeviceConstants.COMMODITY))
			oiAnalysis = Market.getOiAnalysisCommodity(category, session.getAppID());
