...
public class CreateWatchlist extends SessionService {
	...
	if(!(watchListName.equalsIgnoreCase(DeviceConstants.MYHOLD))){
		...
		if (Watchlist.createWatchList(gcRequest.getSession(), gcRequest.getSession().getUserID(),
			...
			gcResponse.addToData(DeviceConstants.MSG, InfoMessage.getInfoMSG("info_msg.watchlist.WATCHLIST_ADDED"));
			gcResponse.addToData(DeviceConstants.IS_VALID_WATCHLIST_NAME ,"true");
		
		}
		}
	else {
		gcResponse.addToData(DeviceConstants.MSG, String.format(InfoMessage.getInfoMSG("info_msg.Watchlist.WATCHLIST_NAME"),watchListName));
