...
import com.globecapital.business.chart.Chart_103;
...
public class GetChartDetails_102  extends BaseService{
	...
	@Override
		...
		String groupInterval = gcRequest.getFromData(DeviceConstants.INTERVAL);
		...
		if( startDate.compareTo(currentDate) == 0 && endDate.compareTo(currentDate) >= 0 ) 
		{
			...
			//Intraday
			JSONArray todayArr = Chart_103.getIntradayChart(sSymbolToken, sStartDateTime, sEndDateTime, sInterval,groupInterval);
		...
		else if( startDate.compareTo(currentDate) < 0 && endDate.compareTo(currentDate) >= 0)
		{
			...
			JSONArray historicalArr = Chart_103.getHistoricalChart(sSymbolToken, sStartDateTime, 
					DateUtils.getPreviousDate(SpyderConstants.DATE_FORMAT) + " 23:59:59",
					sInterval, gcRequest.getAppID(),groupInterval);
			JSONArray todayArr = Chart_103.getIntradayChart(sSymbolToken,
			DateUtils.getCurrentDateTime(SpyderConstants.DATE_FORMAT) + " 00:00:00", 
			sEndDateTime, sInterval,groupInterval);
		...
		else if( startDate.compareTo(currentDate) < 0 && endDate.compareTo(currentDate) < 0)
		{
			...
			JSONArray historicalArr = Chart_103.getHistoricalChart(sSymbolToken, sStartDateTime, 
					sEndDateTime, sInterval, gcRequest.getAppID(),groupInterval);
