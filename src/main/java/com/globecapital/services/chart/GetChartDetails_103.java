...
import com.globecapital.business.chart.Chart_104;
...
public class GetChartDetails_103  extends BaseService{
	...
	@Override
		...
		String exch = symObj.getString(DeviceConstants.EXCH);
		...
		String mktSegId = sSymbolToken.split("_")[1];
		String instrument = "";
		if(SymbolMap.isValidSymbol(sSymbolToken)) {
			SymbolRow symRow = SymbolMap.getSymbolRow(sSymbolToken);
			instrument = symRow.getInstrument();
		...
		if( startDate.compareTo(currentDate) == 0 && endDate.compareTo(currentDate) >= 0 ) 
		{
			...
			//Intraday
			JSONArray todayArr = Chart_104.getIntradayChart(sSymbolToken, sStartDateTime, sEndDateTime, sInterval);
		...
		else if( startDate.compareTo(currentDate) < 0 && endDate.compareTo(currentDate) >= 0)
		{
			...
			{
				ChartUtils.loadChartDate(endDate);
			...
			JSONArray historicalArr;
			if(ChartUtils.getResolution(sInterval) == 1440 && ExchangeSegment.isEquitySegment(mktSegId) && !InstrumentType.isIndex(instrument))
				historicalArr = Chart_104.getHistoricalChart(sSymbolToken, sStartDateTime, 
						DateUtils.getPreviousDate(SpyderConstants.DATE_FORMAT) + " 23:59:59",
						sInterval, gcRequest.getAppID());
			else
				historicalArr = Chart_104.getIntradayChartHistory(sSymbolToken, sStartDateTime, sEndDateTime, sInterval);
			JSONArray todayArr = Chart_104.getIntradayChart(sSymbolToken,
		...
		else if( startDate.compareTo(currentDate) < 0 && endDate.compareTo(currentDate) < 0)
		{
			...
			if(ChartUtils.getResolution(sInterval) == 1440 && ExchangeSegment.isEquitySegment(mktSegId) && !InstrumentType.isIndex(instrument))
				...
				historicalArr =  Chart_104.getIntradayChartHistory(sSymbolToken, sStartDateTime, 
						sEndDateTime, sInterval);
