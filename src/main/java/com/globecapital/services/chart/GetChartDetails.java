...
public class GetChartDetails  extends BaseService{
	...
	@Override
		...
		if( startDate.compareTo(currentDate) == 0 && endDate.compareTo(currentDate) >= 0 ) 
		{
			...
			//Intraday
			JSONArray todayArr = Chart.getIntradayChart(sSymbolToken, sStartDateTime, sEndDateTime, sInterval);
		...
		else if( startDate.compareTo(currentDate) < 0 && endDate.compareTo(currentDate) >= 0)
		{
			...
			JSONArray todayArr = Chart.getIntradayChart(sSymbolToken,
