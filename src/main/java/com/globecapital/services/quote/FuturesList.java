...
public class FuturesList extends BaseService {
	...
	protected void process(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		JSONObject symbObj = gcRequest.getObjectFromData(SymbolConstants.SYMBOL_OBJ);
		JSONArray futures = AdvanceQuote.getFutures(symbObj);
		if(futures.length() > 0)
			gcResponse.addToData(DeviceConstants.FUTURES, futures);
		else
			throw new GCException(InfoIDConstants.DYNAMIC_MSG,
					InfoMessage.getInfoMSG("info_msg.invalid.no_futures"));
