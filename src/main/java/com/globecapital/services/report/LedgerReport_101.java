...
public class LedgerReport_101 extends SessionService {
	...
	protected void doPostProcess(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		if (getLedgerReports.length() > 0) {
			...
			if(segmentType.equalsIgnoreCase(DeviceConstants.EQUITY))
				gcResponse.addToData(DeviceConstants.ADVANCED_FILTER,FilterList_101.getFilterTypes(DeviceConstants.LEDGER_EQ,filterObj));
			else if(segmentType.equalsIgnoreCase(DeviceConstants.COMMODITY))
				gcResponse.addToData(DeviceConstants.ADVANCED_FILTER,FilterList_101.getFilterTypes(DeviceConstants.LEDGER_COMMODITY,filterObj));
		} else
