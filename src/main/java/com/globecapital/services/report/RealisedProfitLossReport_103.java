...
public class RealisedProfitLossReport_103 extends SessionService {
	...
	protected void doPostProcess(GCRequest gcRequest, GCResponse gcResponse) throws Exception {
		...
		JSONArray getProfitLossReports = ProfitLoss_101.getRealisedProfitLossReports_101(session, segmentType,
		...
		if (getProfitLossReports.length() > 0) {
			...
			if(segmentType.equalsIgnoreCase(DeviceConstants.EQUITY))
				gcResponse.addToData(DeviceConstants.ADVANCED_FILTER,FilterList_102.getFilterTypes(DeviceConstants.REALISED_PROFIT_LOSS_EQUITY,filterObj));
			else if(segmentType.equalsIgnoreCase(DeviceConstants.DERIVATIVE))
				gcResponse.addToData(DeviceConstants.ADVANCED_FILTER,FilterList_102.getFilterTypes(DeviceConstants.REALISED_PROFIT_LOSS_DERIVATIVE,filterObj));
			else if(segmentType.equalsIgnoreCase(DeviceConstants.CURRENCY))
				gcResponse.addToData(DeviceConstants.ADVANCED_FILTER,FilterList_102.getFilterTypes(DeviceConstants.REALISED_PROFIT_LOSS_CURRENCY,filterObj));
			else if(segmentType.equalsIgnoreCase(DeviceConstants.COMMODITY))
				gcResponse.addToData(DeviceConstants.ADVANCED_FILTER,FilterList_102.getFilterTypes(DeviceConstants.REALISED_PROFIT_LOSS_COMMODITY,filterObj));
			} else
