...
public class Holdings_101 {

    private static Logger log = Logger.getLogger(Holdings_101.class);

    public static JSONArray getHoldingsReports(Session session, JSONObject filterObj, String reportType)
            throws JSONException, SQLException, RequestFailedException {
        ...
        try {
            ...
            if(previousMarketValue!=0)
            	profitLossPercent = (Double.parseDouble(String.valueOf(new BigDecimal(String.valueOf(marketValue)).subtract(new BigDecimal(String.valueOf(previousMarketValue))))) / previousMarketValue) * 100;
        ...
        for (int i = 0; i < holdingsList.length(); i++) {
            if (!holdingsList.getJSONObject(i).toString().contains(DeviceConstants.TOTAL_SUMMARY)) {
                ...
                if (getQuoteDetails.containsKey(sSymbolToken)) {
                    ...
                    holdingsObject.put(DeviceConstants.CHANGE_PERCENT, Math.abs(Double.parseDouble(quoteDetails.sChangePercent)));
                    holdingsObject.put(DeviceConstants.PREV_CLOSE, PriceFormat.priceToRupees(quoteDetails.sPreviousClose));
                    double quantity = Double.parseDouble(holdingsObject.getString(DeviceConstants.NET_QTY));
                    if(!quoteDetails.sLTP.equals("0")) {
            ...
            for (int i = 0; i < holdingsList.length(); i++) {
                ...
                if (!holdingsList.getJSONObject(i).toString().contains(DeviceConstants.TOTAL_SUMMARY)) {
                    ...
                    if (getQuoteDetails.containsKey(sSymbolToken)) {
                        ...
                        unrealisedPL = String.valueOf(new BigDecimal(String.valueOf(marketValue)).subtract(new BigDecimal(String.valueOf(investmentValue))));
                        ...
                        if (!(quoteDetails.sPreviousClose.equals("0") || quoteDetails.sLTP.equals("0"))) {
                            dayPL = Double.parseDouble(String.valueOf(new BigDecimal(quoteDetails.sLTP).subtract(new BigDecimal(quoteDetails.sPreviousClose)))) * Double.parseDouble(qty);
                            dayPLPercent = (((Double.parseDouble(String.valueOf(new BigDecimal(quoteDetails.sLTP).subtract(new BigDecimal(quoteDetails.sPreviousClose)))))
    ...
    public static JSONArray getFilteredHoldings(JSONArray holdingsArray, JSONArray filterBy, String sortOrder,
        ...
        try {
            ...
            if (filterBy.length() == 0) {
                ...
                return sort(holdingsArray, sortOrder, sortBy);
    ...
    public static JSONArray sort(JSONArray holdingsArray, final String sortOrder, String sortBy) {
        JSONArray sortedArray = new JSONArray();
        if (sortBy.contains(GCConstants.MKT_VAL)) {
        	...
        	List<JSONObject> toBeSorted = new ArrayList<JSONObject>();
    		for (int i = 0; i < holdingsArray.length(); i++) {
    			if(!holdingsArray.getJSONObject(i).getString(DeviceConstants.UNREALISED_PROFIT_LOSS).equals("--")) 
    				toBeSorted.add(holdingsArray.getJSONObject(i));
    		}
        ...
        else if (sortBy.contains(GCConstants.ALPHABETICALLY) || sortBy.length()==0) {
				List<JSONObject> toBeSorted = new ArrayList<JSONObject>();
                for (int i = 0; i < holdingsArray.length(); i++)
                    toBeSorted.add(holdingsArray.getJSONObject(i));
