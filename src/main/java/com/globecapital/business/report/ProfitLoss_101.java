...
public class ProfitLoss_101 {
	...
	public static JSONArray getRealisedProfitLossReports(Session session, String segmentType, JSONObject filterObj,
			String reportType) throws JSONException, RequestFailedException {
		...
		if(filterObj.getString(DeviceConstants.TO_DATE).isEmpty() && filterObj.getString(DeviceConstants.FROM_DATE).isEmpty()){
			Calendar startCal = Calendar.getInstance();
			startCal.add(Calendar.MONTH, -1);
			Date startDate;
			startDate = startCal.getTime();
			SimpleDateFormat destinationFormat = new SimpleDateFormat(DeviceConstants.REPORT_DATE_FORMAT);
			filterObj.put( DeviceConstants.TO_DATE, DateUtils.getCurrentDateTime(DeviceConstants.REPORT_DATE_FORMAT));
			filterObj.put( DeviceConstants.FROM_DATE, destinationFormat.format(startDate));
		}
		try {
...
//			profitLossRequest.setToken(GCAPIAuthToken.getAuthToken());
			...
			for (GetRealisedPLRows rows : profitLossRows) {
				...
				profitLossObj.put(DeviceConstants.SYMBOL_DETAILS,scripName
				...
				transApiObj.put(DeviceConstants.SEGMENT_TYPE, segmentType);
				transApiObj.put(DeviceConstants.SCRIP_NAME, scripName);
	...
	public static JSONArray getRealisedProfitLossReports_101(Session session, String segmentType, JSONObject filterObj,
			String reportType) throws JSONException, RequestFailedException {
...
//			profitLossRequest.setToken(GCAPIAuthToken.getAuthToken());
			...
			for (GetRealisedPLRows rows : profitLossRows) {
				...
				if(segmentType.equals(DeviceConstants.CURRENCY)) {
					...
					profitLossObj.put(DeviceConstants.BUY_QTY,  String.valueOf(Integer.parseInt(rows.getBuyQty()) / lotsize));
					profitLossObj.put(DeviceConstants.SELL_QTY, String.valueOf(Integer.parseInt(rows.getSellQty()) / lotsize));
				}else {
					profitLossObj.put(DeviceConstants.BUY_QTY, rows.getBuyQty());
					profitLossObj.put(DeviceConstants.SELL_QTY, rows.getSellQty());
				}
...
//			profitLossRequest.setToken(GCAPIAuthToken.getAuthToken());
			...
			for (GetUnrealisedPLRows rows : unrealisedProfitLossRows) {
					...
					if (segmentType.equalsIgnoreCase(DeviceConstants.EQUITY)) {
						...
						else if(filterBy.toString().contains(GCConstants.FUTURES) &&!rows.getInstrument().equals(InstrumentType.FUTURES))
						{
						continue;
						}
						else if(filterBy.toString().contains(GCConstants.OPTIONS) &&!rows.getInstrument().equals(InstrumentType.OPTIONS))
						{
						continue;
						}
						SymbolRow symbolProfitLossObj = new SymbolRow();
...
//			getPrevClose(profitLossArray, linkedsetSymbolToken, precision);
	...
	public static JSONArray getUnRealisedProfitLossReports_101(Session session, String segmentType, JSONObject filterObj)
			throws JSONException, RequestFailedException {
...
//			profitLossRequest.setToken(GCAPIAuthToken.getAuthToken());
			...
			for (GetUnrealisedPLRows rows : unrealisedProfitLossRows) {
					...
					if (segmentType.equalsIgnoreCase(DeviceConstants.EQUITY)) {
						...
						lotSize = symbolProfitLossObj.getLotSizeInt();
						exchange = symbolProfitLossObj.getExchange();
					...
					if(exchange.equalsIgnoreCase(ExchangeSegment.NSECDS)) {
						profitLossObj.put(DeviceConstants.OPEN_QTY, String.valueOf(Math.abs(Integer.parseInt(rows.getOpenQty())/lotSize)));
					}else {
						profitLossObj.put(DeviceConstants.OPEN_QTY, String.valueOf(Math.abs(Integer.parseInt(rows.getOpenQty()))));
					}
					profitLossObj.put(DeviceConstants.MARKET_VALUE,
...
//				equityTransactionScripRequest.setToken(GCAPIAuthToken.getAuthToken());
	...
	public static JSONArray sort(JSONArray toBeSortedArray, final String sortOrder, String sortBy) {
        ...
        if (sortBy.contains(GCConstants.PROFIT_N_LOSS)) {
			List<JSONObject> toBeSorted = new ArrayList<JSONObject>();
			if(toBeSortedArray.getJSONObject(0).has(DeviceConstants.REALISED_PROFIT_LOSS)) {
				for (int i = 0; i < toBeSortedArray.length(); i++) {
					if(!toBeSortedArray.getJSONObject(i).getString(DeviceConstants.REALISED_PROFIT_LOSS).equals("--")) 
						toBeSorted.add(toBeSortedArray.getJSONObject(i));
				}
				SortHelper.sortByDouble(DeviceConstants.REALISED_PROFIT_LOSS,toBeSorted,"[,\u20B9]");
				if (sortOrder.contains(DeviceConstants.ASCENDING) || sortOrder.isEmpty())
				...
				for (int i = 0; i < toBeSortedArray.length(); i++) {
					if(!toBeSortedArray.getJSONObject(i).getString(DeviceConstants.UNREALISED_PROFIT_LOSS).equals("--")) 
						toBeSorted.add(toBeSortedArray.getJSONObject(i));
				}
				SortHelper.sortByDouble(DeviceConstants.UNREALISED_PROFIT_LOSS,toBeSorted,"[,\u20B9]");
				if (sortOrder.contains(DeviceConstants.ASCENDING) || sortOrder.contains(DeviceConstants.DEFAULT_ORDER))
					sorted = new JSONArray(toBeSorted);
				else {
					Collections.reverse(toBeSorted);
					sorted = new JSONArray(toBeSorted);	
