...
public class Transaction_101 {

	private static Logger log = Logger.getLogger(Transaction_101.class);
		...
		try {
			...
			if (segmentType.equalsIgnoreCase(DeviceConstants.EQUITY)) {
				...
				if(equityTransactionResponse.getMsg().equalsIgnoreCase(MessageConstants.SESSION_EXPIRED)) {
					transactionRequest.setToken(GCAPIAuthToken.getAuthToken());
					equityTransactionResponse = equityTransactionAPI.get(transactionRequest, GetEquityTransactionResponse.class, session.getAppID(),"GetEquityTransaction");
				}
			...
			Collections.reverse(transactionRows);
			for (GetTransactionRows rows : transactionRows) {
				String instType="";
				if(segmentType.equalsIgnoreCase(DeviceConstants.DERIVATIVE)) {
					instType=rows.getInst();
				}else {
					instType=rows.getInstrument();
				}
				if(filterBy.length() == 0 || (filterBy.toString().contains(GCConstants.FUTURES) && (filterBy.toString().contains(GCConstants.OPTIONS))))
				{
				//Do nothing
				}
				else if(filterBy.toString().contains(GCConstants.FUTURES) &&!InstrumentType.isFutures(instType))
				{
				continue;
				}
				else if(filterBy.toString().contains(GCConstants.OPTIONS) &&!InstrumentType.isOptions(instType))
				{
				continue;
				}
	...
	public static JSONArray getTransactionReports_101(Session session, String segmentType, JSONObject filterObj,
			String reportType) throws JSONException, RequestFailedException {
			...
			for (GetTransactionRows rows : transactionRows) {
				...
				continue;
					...
					if(segmentType.equalsIgnoreCase(DeviceConstants.CURRENCY)) {
						...
						transactionObj.put(DeviceConstants.QTY, String.valueOf(Math.abs(Integer.parseInt(rows.getQty())/lotsize)));
					}else {
						transactionObj.put(DeviceConstants.QTY, String.valueOf(Math.abs(Integer.parseInt(rows.getQty()))));
					}
					if (rows.getBs().equalsIgnoreCase("B")) {
	...
	public static JSONArray getTransactionArray(JSONArray transactionArray, JSONArray filterBy, String sortOrder,
		...
		try {
			...
			if (filterBy.toString().contains(DeviceConstants.BUY_FILTER) 
					&& !filterBy.toString().contains(DeviceConstants.SELL_FILTER)) {
				...
				return sortedArray;
					...
					&& !filterBy.toString().contains(DeviceConstants.BUY_FILTER)) {
				...
				return sortedArray=sort(transactionArray, sortOrder, sortBy);
			}
	...
	public static JSONArray sort(JSONArray finalTransactionArray, final String sortOrder, String sortBy) {
		...
		try {
			...
			if (sortBy.contains(DeviceConstants.QUANTITY)) {
				return SortHelper.sortByInteger(finalTransactionArray, sortOrder, DeviceConstants.QTY);
			} else if (sortBy.contains(DeviceConstants.DATE)) {
				return SortHelper.sortByTransDate(finalTransactionArray, sortOrder, DeviceConstants.REPORT_DATE);	
			} else if (sortBy.contains(GCConstants.ALPHABETICALLY) || sortBy.isEmpty()) {
				if(sortBy.isEmpty()) {
					return SortHelper.sortBySymbol(finalTransactionArray,DeviceConstants.ASCENDING);
				}else {
