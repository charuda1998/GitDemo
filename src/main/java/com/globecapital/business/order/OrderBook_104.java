...
public class OrderBook_104 {

	private static Logger log = Logger.getLogger(OrderBook_104.class);
	...
	public static JSONObject getOrderBook(List<GetOrderBookObjectRow> orderRows, List<GetTradeBookObjectRow> tradeRows,
		...
		switch(sOrderStatus) {
			case OrderConstants.PENDING:
				gtdOrderBookRow = gtdOrderBook.getGTDPendingSummary();
				getGTDOrdersCount(gtdOrderBook, orderBookObj);
				break;
			case OrderConstants.EXECUTED:
				gtdOrderBookRow = gtdOrderBook.getGTDExecutedSummary();
				pendingGTDOrderBookRow = gtdOrderBook.getGTDPendingSummary();
				getGTDOrdersCount(gtdOrderBook, orderBookObj);
				break;
			case OrderConstants.CANCELLED:
                gtdOrderBookRow = gtdOrderBook.getGTDCancelledSummary();
                pendingGTDOrderBookRow = gtdOrderBook.getGTDPendingSummary();
                getGTDOrdersCount(gtdOrderBook, orderBookObj);
                break;
			default:
			    gtdOrderBookRow = gtdOrderBook.getGTDOrderSummary();
				pendingGTDOrderBookRow = gtdOrderBook.getGTDPendingSummary();
				getGTDOrdersCount(gtdOrderBook, orderBookObj);
				break;
		...
		// Parsed bracket order map - Main Order
		Map<String, SymbolRow> mapBracketOrdersParsed = new HashMap<String, SymbolRow>();
		
		// Parsed bracket order map - square off Order
		Map<String, SymbolRow> mapBracketOrdersParsedSq = new HashMap<String, SymbolRow>();

		// Parsed bracket order map - SL Order
		Map<String, SymbolRow> mapBracketOrdersParsedSL = new HashMap<String, SymbolRow>();

		// Parsed bracket order map - Target Order
		Map<String, SymbolRow> mapBracketOrdersParsedT = new HashMap<String, SymbolRow>();

		// Parsed Cover Order
		Map<String, SymbolRow> mapCoverOrdersParsed = new HashMap<String, SymbolRow>();
		
		// Parsed Cover Order Leg
        Map<String, SymbolRow> mapCoverLegOrdersParsed = new HashMap<String, SymbolRow>();
        
        // Parsed Cover Order Leg
        Map<String, SymbolRow> mapCoverSquareOffOrdersParsed = new HashMap<String, SymbolRow>();
		...
		// Contains array of executed bracket orders based on bracket order ID
		Map<String, JSONArray> mapExeBracketOrders = new HashMap<String, JSONArray>();

		// Contains array of bracket order with square off based on bracket order ID
		Map<String, JSONArray> mapSquareOffBracketOrders = new HashMap<String, JSONArray>();
		
		// Contains array of cover order based on recoId
		Map<String, JSONArray> mapCoverOrders = new HashMap<String, JSONArray>();
		
		// Contains array of square off cover order based on recoId
        Map<String, JSONArray> mapSquareOffCoverOrders = new HashMap<String, JSONArray>();
        
        // Contains array of bracket order based on recoId
        Map<String, JSONArray> mapExeCoverOrders = new HashMap<String, JSONArray>();
        
        List<GetOrderBookObjectRow> exceptionHandled = new ArrayList();
		...
		for (int i = 0; i < orderRows.size(); i++) {
			try {
				...
				if (!orderRow.getBracketOrdId().isEmpty()  && !orderRow.getBracketOrdId().equals("0")) { // Bracket order
					...
					switch (orderRow.getLegIndicator()) {
					case OrderConstants.MAIN_LEG_INDICATOR_9:
						order = getNormalOrderDetails(orderRow, linkedsetSymbolToken, "All", orderBookObj,
								gtdOrderBookRow);
						break;
					...
					continue; // bracket orders should be parsed once we done
						...
						&& orderRow.getProdType().equalsIgnoreCase(FTConstants.MARGIN_PLUS) && orderRow.getRecoId().length() > 13) { // Cover order
					/*** If it is main leg, get normal order details also ***/
					String recoId = getRecoID(orderRow.getRecoId());
					orderRow.setRecoId(recoId);

					if (   (orderRow.getLegIndicator().equals("0"))
							? orderRow.getOrderType().equalsIgnoreCase(FTConstants.RL_MKT) : orderRow.getLegIndicator().equals(FTConstants.ONE_FLAG) )
							   //||    ( (orderRow.getLegIndicator().equals("0")) && !mapCoverOrders.containsKey(orderRow.getRecoId().split(" ")[0]) ) ) 
					{
						SymbolRow order = getNormalOrderDetails(orderRow, linkedsetSymbolToken, "All",
					...
					if (mapCoverOrders.containsKey(recoId)) {
						mapCoverOrders.get(recoId).put(orderRow);
					} else {
						JSONArray arr = new JSONArray();
						arr.put(orderRow);
						mapCoverOrders.put(recoId, arr);
					}
		...
		/*** to get details of all the bracket orders. passes two map to get executed bracket order and bracket order with square off ***/
		// TODO: Temp fix, need to uncomment when bracket order approved for live 
		mapBracketOrdersParsed = getBracketOrderDetails(mapBracketOrders, linkedsetSymbolToken, sOrderStatus, mapExeBracketOrders, mapSquareOffBracketOrders, orderBookObj);

		/*** to get details of all the bracket stoploss orders ***/
		mapBracketOrdersParsedSL = getBracketOrderSLDetails(mapExeBracketOrders,linkedsetSymbolToken, sOrderStatus);

		/*** to get details of all the bracket profit orders ***/
		mapBracketOrdersParsedT = getBracketOrderPDetails(mapExeBracketOrders,linkedsetSymbolToken, sOrderStatus);

		/*** to get details of all bracket square off details ***/
		mapBracketOrdersParsedSq = getBracketOrderSquareOffDetails(mapSquareOffBracketOrders, linkedsetSymbolToken, sOrderStatus);
		
		//to get details of all the cover orders. pass 2 map to get executed cover order and cover order with square off
		mapCoverOrdersParsed = getCoverOrderDetails(mapCoverOrders, linkedsetSymbolToken, sOrderStatus, mapExeCoverOrders, mapSquareOffCoverOrders, orderBookObj);

		//to get details of cover order with respect to leg order
		mapCoverLegOrdersParsed = getCoverLegOrderDetails(mapExeCoverOrders, linkedsetSymbolToken, sOrderStatus);

		//to get details of cover order with respect to square off order
		mapCoverSquareOffOrdersParsed = getCoverSquareOffOrderDetails(mapSquareOffCoverOrders, linkedsetSymbolToken, sOrderStatus);
	     /***
		...
		while (iterator.hasNext()) {
			...
			boolean isBracketOrder = false, isCoverOrder = false;
			if (mapBracketOrdersParsedSL.containsKey(order.getString(OrderConstants.BRACKET_ORDER_ID))) {
				orderList.put(mapBracketOrdersParsedSL.get(order.getString(OrderConstants.BRACKET_ORDER_ID)));
				isBracketOrder = true;
			}
			if (mapBracketOrdersParsedSq.containsKey(order.getString(OrderConstants.BRACKET_ORDER_ID))) {
                orderList.put(mapBracketOrdersParsedSq.get(order.getString(OrderConstants.BRACKET_ORDER_ID)));
                isBracketOrder = true;
            }
			if (mapBracketOrdersParsedT.containsKey(order.getString(OrderConstants.BRACKET_ORDER_ID))) {
				orderList.put(mapBracketOrdersParsedT.get(order.getString(OrderConstants.BRACKET_ORDER_ID)));
				isBracketOrder = true;
			}
			if (mapCoverLegOrdersParsed.containsKey( (order.has(OrderConstants.RECO_ID)) ? order.getString(OrderConstants.RECO_ID) : null)) {
                orderList.put(mapCoverLegOrdersParsed.get(order.getString(OrderConstants.RECO_ID)));
                isCoverOrder = true;
            }
			
			if (mapCoverSquareOffOrdersParsed.containsKey( (order.has(OrderConstants.RECO_ID)) ? order.getString(OrderConstants.RECO_ID) : null)) {
                orderList.put(mapCoverSquareOffOrdersParsed.get(order.getString(OrderConstants.RECO_ID)));
                isCoverOrder = true;
            }
			...
			if (mapBracketOrdersParsed.containsKey(order.getString(OrderConstants.BRACKET_ORDER_ID))) {
			    ...
			    if(!isBracketOrder && order.getString(OrderConstants.BRACKET_ORDER_ID).isEmpty() && !isCoverOrder 
			            && !order.has(OrderConstants.RECO_ID))
			        orderList.put(order);
			    else if(order.getString(OrderConstants.BRACKET_ORDER_ID).equals("0")) {
			    	orderList.put(order);
			    }else if(!isCoverOrder && order.has(OrderConstants.RECO_ID)) {
			        String recoId = order.getString(OrderConstants.RECO_ID);
			        if(recoId.length() < 13 )
		                 orderList.put(order);
			    }
			}
	...
	private static Map<String, SymbolRow> getBracketOrderDetails(Map<String, JSONArray> mapBracketOrders,
			LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus, Map<String, JSONArray> mapExeBracketOrders, Map<String, JSONArray> mapSqaureOffBracketOrders, JSONObject orderBook)
			throws Exception {
		...
		for (Map.Entry<String, JSONArray> entry : mapBracketOrders.entrySet()) {
			...
			order.put(OrderConstants.BRACKET_ORDER_DETAILS, getBracketOrderDetails(entry.getValue(), order, mapExeBracketOrders, mapSqaureOffBracketOrders, orderBook));
	...
	private static Map<String, SymbolRow> getBracketOrderSquareOffDetails(Map<String, JSONArray> mapBracketOrders,
        ...
        for (Map.Entry<String, JSONArray> entry : mapBracketOrders.entrySet()) {
            ...
            String sTokenMktSegID = sScripCode + "_" + sMktSegID;
            order.extend(SymbolMap.getSymbolRow(sScripCode, sMktSegID).getMinimisedSymbolRow());
            order.put(OrderConstants.BRACKET_ORDER_DETAILS, getBracketSquareOffOrderDetails(entry.getValue(), order));
            order.put(OrderConstants.IS_BRACKET_ORDER, "true");
	...
	/*** Returns parsed bracket order details map, key - bracket order id ***/
	private static Map<String, SymbolRow> getBracketOrderSLDetails(Map<String, JSONArray> mapBracketOrders,
			LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus) throws Exception {
		...
		for (Map.Entry<String, JSONArray> entry : mapBracketOrders.entrySet()) {
			...
			String sTokenMktSegID = sScripCode + "_" + sMktSegID;
			order.extend(SymbolMap.getSymbolRow(sScripCode, sMktSegID).getMinimisedSymbolRow());
			order.put(OrderConstants.BRACKET_ORDER_DETAILS, getBracketSLOrderDetails(entry.getValue(), order));
			order.put(OrderConstants.IS_BRACKET_ORDER, "true");
			order.put(OrderConstants.IS_BRACKET_MAIN_ORDER, "false");
	...
	private static Map<String, SymbolRow> getBracketOrderPDetails(Map<String, JSONArray> mapBracketOrders,
			LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus) throws Exception {
		...
		for (Map.Entry<String, JSONArray> entry : mapBracketOrders.entrySet()) {
			...
			order.put(OrderConstants.BRACKET_ORDER_DETAILS, getBracketProfitOrderDetails(entry.getValue(), order));
			order.put(OrderConstants.IS_BRACKET_ORDER, "true");
			order.put(OrderConstants.IS_BRACKET_MAIN_ORDER, "false");
            order.put(OrderConstants.IS_BRACKET_STOPLOSS_ORDER, "false");
            order.put(OrderConstants.IS_BRACKET_SQUAREOFF_ORDER, "false");
            order.put(OrderConstants.IS_BRACKET_PROFIT_ORDER, "true");
	...
	private static SymbolRow getNormalOrderDetails(GetOrderBookObjectRow orderRow,
		...
		//Set CO and BO square-off as intraday order
		if((!orderRow.getRecoId().isEmpty() && orderRow.getProdType().equalsIgnoreCase(FTConstants.MARGIN_PLUS))
		        || !orderRow.getBracketOrdId().isEmpty())
		    order.put(OrderConstants.PRODUCT_TYPE, ProductType.formatToDisplay(ProductType.FT_INTRADAY_FULL_TEXT,
		...
		if (isGTD) {// To Identify GTD Order
			...
			if (gtdOrderBookRow.containsKey(orderRow.getGatewayOrdNo())) {
		        ...
		        order.put(OrderConstants.DISP_STATUS, OrderStatus.getDispGTDOrderStatus(gtdDetails.getGTDOrderStatus()));
			}
		} else
		    ...
		    if (sStatus.equalsIgnoreCase(OrderConstants.PENDING)) {
    			orderBookObj.put(DeviceConstants.PENDING_COUNT,
    					orderBookObj.has(DeviceConstants.PENDING_COUNT)
    							? String.valueOf(
    									Integer.parseInt(orderBookObj.getString(DeviceConstants.PENDING_COUNT)) + 1): "1");
    			...
    			orderBookObj.put(DeviceConstants.EXECUTED_COUNT,
    					orderBookObj.has(DeviceConstants.EXECUTED_COUNT)
    							? String.valueOf(
    									Integer.parseInt(orderBookObj.getString(DeviceConstants.EXECUTED_COUNT)) + 1): "1");
                ...
                orderBookObj.put(DeviceConstants.CANCELLED_COUNT,
                        orderBookObj.has(DeviceConstants.CANCELLED_COUNT)
                                ? String.valueOf(
                                        Integer.parseInt(orderBookObj.getString(DeviceConstants.CANCELLED_COUNT)) + 1): "1");
            }
		}
		/*** To set, modifiable, repeat and cancellable flags ***/
        ...
        order.put(OrderConstants.IS_REPEAT_ORDER, sStatus.equals(OrderConstants.CANCELLED) ? "true" : "false" );
	...
	private static void getOrderDetails(GetOrderBookObjectRow orderRow, SymbolRow order) throws Exception {
		...
		if (!orderRow.getBracketOrdId().isEmpty()) {
		    ...
		    JSONObject coverPrdType = new JSONObject();
		    coverPrdType.put(DeviceConstants.PRODUCT_TYPE, ProductType.formatToDisplay(orderRow.getProdType(),
                    ExchangeSegment.getMarketSegmentID(orderRow.getExch(), orderRow.getInst())));
		    coverPrdType.put(DeviceConstants.DISP_PRODUCT_TYPE, OrderConstants.INTRADAY);
            order.put(OrderConstants.PRODUCT_TYPE, coverPrdType);
		} else
		...
		order.put("legIndicator", orderRow.getLegIndicator());
	}
	...
	private static JSONObject getBracketOrderDetails(JSONArray brackerOrders, SymbolRow order, Map<String, JSONArray> mapExeBracketOrders, Map<String, JSONArray> mapSqaureOffBracketOrders, JSONObject orderBookObj)
		...
		String isOrderWithLeg = ""; // get Bracket Order Id of Bracket order with leg order
		...
		for (int i = 0; i < brackerOrders.length(); i++) {
			...
			if(!orderRow.getLegIndicator().equals(OrderConstants.MAIN_LEG_INDICATOR_9)) {
			  String sStatus =   OrderStatus.getStatus(orderRow.getOrdStat());
    		   if (sStatus.equalsIgnoreCase(OrderConstants.PENDING)) {
    	            orderBookObj.put(DeviceConstants.PENDING_COUNT, orderBookObj.has(DeviceConstants.PENDING_COUNT)
    	                    ? String.valueOf(Integer.parseInt(orderBookObj.getString(DeviceConstants.PENDING_COUNT)) + 1) : "1");
    	        } else if (sStatus.equalsIgnoreCase(OrderConstants.EXECUTED)) {
    	            orderBookObj.put(DeviceConstants.EXECUTED_COUNT, orderBookObj.has(DeviceConstants.EXECUTED_COUNT)
    	                            ? String.valueOf(Integer.parseInt(orderBookObj.getString(DeviceConstants.EXECUTED_COUNT)) + 1) : "1");
    	        }  else if (sStatus.equalsIgnoreCase(OrderConstants.CANCELLED)) {
                    orderBookObj.put(DeviceConstants.CANCELLED_COUNT, orderBookObj.has(DeviceConstants.CANCELLED_COUNT)
                                    ? String.valueOf(Integer.parseInt(orderBookObj.getString(DeviceConstants.CANCELLED_COUNT)) + 1) : "1");
                }
			}
			...
			if (orderRow.getLegIndicator().equals(OrderConstants.MAIN_LEG_INDICATOR_9)) {
				...
				orderAction = OrderAction.formatToDevice(orderRow.getBuySell());
			    ...
			    isOrderWithLeg = orderRow.getBracketOrdId();
			...
			{
			    isOrderWithLeg = orderRow.getBracketOrdId();
			    mapSqaureOffBracketOrders.put(orderRow.getBracketOrdId(), brackerOrders);
		...
		if(!isOrderWithLeg.isEmpty())
		    mapExeBracketOrders.put(isOrderWithLeg, brackerOrders);
		...
		order.put(OrderConstants.IS_MODIFIABLE, Boolean.
		        toString(OrderStatus.isBracketOrderModifiable_101(sMainOrderStatus, sSLOrderStatus, sProfitOrderStatus, OrderConstants.IS_BRACKET_MAIN_ORDER)));
		order.put(OrderConstants.IS_CANCELLABLE, Boolean
				.toString(OrderStatus.isBracketOrderModifyOrCancellable(sMainOrderStatus, sSLOrderStatus, sProfitOrderStatus,true)));
		order.put(OrderConstants.EXIT_OPTION, false);//Boolean.toString(OrderStatus.getBracketOrderExitOption_101(sMainOrderStatus, sSLOrderStatus, true)));
		
		//Repeat Order
		order.put(OrderConstants.IS_REPEAT_ORDER, sMainOrderStatus.equals(OrderConstants.CANCELLED) ? "true" : "false" );
		//OrderAction for leg orders
		order.put(OrderConstants.BRACKET_ORDER_ACTION, orderAction);
		...
		OrderStatus.getBuyOrSellMoreFlags_BO(sMainOrderStatus, sSLOrderStatus, sProfitOrderStatus, sSquareoffStatus,
				sBuyOrSell, order, OrderConstants.IS_BRACKET_MAIN_ORDER);
	...
	private static Map<String, SymbolRow> getCoverOrderDetails(Map<String, JSONArray> mapCoverOrders,
			LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus, Map<String, JSONArray> mapExeCoverOrders, 
			Map<String, JSONArray> mapSqaureOffCoverOrders, JSONObject orderBook) throws Exception {
		...
		for (Map.Entry<String, JSONArray> entry : mapCoverOrders.entrySet()) {
			...
			String sTokenMktSegID = sScripCode + "_" + sMktSegID;
			order.extend(SymbolMap.getSymbolRow(sScripCode, sMktSegID).getMinimisedSymbolRow());
			order.put(FTConstants.CO_DETAILS, getCoverOrderDetails(entry.getValue(), order, mapExeCoverOrders, mapSqaureOffCoverOrders, orderBook));
			...
			String sStatus = "";
			try {
				sStatus = OrderStatus.getStatus(order.getString(OrderConstants.STATUS));
			}catch (Exception e) {
				log.error(e);
				continue;
			}
	...
	private static Map<String, SymbolRow> getCoverLegOrderDetails(Map<String, JSONArray> mapCoverOrders,
            LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus) throws Exception {
        ...
        for (Map.Entry<String, JSONArray> entry : mapCoverOrders.entrySet()) {
            ...
            order.put(FTConstants.CO_DETAILS, getCoverLegOrderDetails(entry.getValue(), order));
            order.put(OrderConstants.IS_COVER_ORDER, "true");
	...
	private static Map<String, SymbolRow> getCoverSquareOffOrderDetails(Map<String, JSONArray> mapCoverOrders,
            LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus) throws Exception {
        ...
        for (Map.Entry<String, JSONArray> entry : mapCoverOrders.entrySet()) {
            ...
            String sTokenMktSegID = sScripCode + "_" + sMktSegID;
            order.extend(SymbolMap.getSymbolRow(sScripCode, sMktSegID).getMinimisedSymbolRow());
            order.put(FTConstants.CO_DETAILS, getCoverSquareOffOrderDetails(entry.getValue(), order));
            order.put(OrderConstants.IS_COVER_ORDER, "true");
            order.put(OrderConstants.IS_COVER_MAIN_ORDER, "false");
            order.put(OrderConstants.IS_COVER_SL_ORDER, "false");
	...
	private static JSONObject getCoverOrderDetails(JSONArray coverOrders, SymbolRow order, Map<String, JSONArray> mapExeCoverOrders, 
            Map<String, JSONArray> mapSqaureOffCoverOrders, JSONObject orderBook)
			throws JSONException, Exception {

		JSONObject coverOrdObj = new JSONObject(); // Final Object contains bracket order details
		JSONObject legOrderObj = new JSONObject(); // Leg order short detail
		JSONArray coverOrderArr = new JSONArray(); // Array of pending and executed order details
		JSONArray cancelledLegOrderArr = new JSONArray(); // Array of cancelled order details
		JSONObject modifyOrderDetailsObj = new JSONObject(); // To be forwarded while sending bracket order modify order
		...
		for (int i = 0; i < coverOrders.length(); i++) {
			...
			if(!((orderRow.getLegIndicator().equals("0")) ? orderRow.getOrderType().equalsIgnoreCase(FTConstants.RL_MKT)
                    : orderRow.getLegIndicator().equals("1"))) {
	              String sStatus =   OrderStatus.getStatus(orderRow.getOrdStat());
	               if (sStatus.equalsIgnoreCase(OrderConstants.PENDING)) {
	                   orderBook.put(DeviceConstants.PENDING_COUNT, orderBook.has(DeviceConstants.PENDING_COUNT)
	                            ? String.valueOf(Integer.parseInt(orderBook.getString(DeviceConstants.PENDING_COUNT)) + 1) : "1");
	                } else if (sStatus.equalsIgnoreCase(OrderConstants.EXECUTED)) {
	                    orderBook.put(DeviceConstants.EXECUTED_COUNT, orderBook.has(DeviceConstants.EXECUTED_COUNT)
	                                    ? String.valueOf(Integer.parseInt(orderBook.getString(DeviceConstants.EXECUTED_COUNT)) + 1) : "1");
	                } else if (sStatus.equalsIgnoreCase(OrderConstants.CANCELLED)) {
                        orderBook.put(DeviceConstants.CANCELLED_COUNT, orderBook.has(DeviceConstants.CANCELLED_COUNT)
                                        ? String.valueOf(Integer.parseInt(orderBook.getString(DeviceConstants.CANCELLED_COUNT)) + 1) : "1");
                    }
	            }
			if ((orderRow.getLegIndicator().equals("0")) ? orderRow.getOrderType().equalsIgnoreCase(FTConstants.RL_MKT)
					: orderRow.getLegIndicator().equals("1")) {
...
//					coverOrderArr.put(slOrderObj);
				...
				if (sSLOrderStatus.equals(OrderConstants.PENDING)) {
					...
					modifyOrderDetailsObj.put(OrderConstants.SL_PRICE, orderRow.getPrc());
					modifyOrderDetailsObj.put(OrderConstants.SL_TRIG_PRICE, orderRow.getTrigPrc());
					modifyOrderDetailsObj.put(OrderConstants.IS_SL_ORDER_MODIFIABLE, "true");
					modifyOrderDetailsObj.put(OrderConstants.SL_ORDER_MODIFY, getModifyOrderDetails(orderRow, false)
							.put(FTConstants.CLIENT_ORD_NO, orderRow.getClientOrdNo()));
				}
		...
		order.put(OrderConstants.IS_CANCELLABLE,
				Boolean.toString(OrderStatus.isCoverOrderCancellable(sMainOrderStatus, sSLOrderStatus, true)));
		...
		// Temporary fix
		order.put(OrderConstants.CONVERT_OPTION, "false");
		order.put(OrderConstants.COVER_ORDER_ACTION, orderAction);

		/*** To show, leg order short description status ***/

		legOrderObj.put(OrderConstants.IS_EXECUTED,
				Boolean.toString(OrderStatus.checkAnyLegOrderExecuted(sSLOrderStatus, "", sSquareoffStatus)));
		legOrderObj.put(OrderConstants.DISP_STATUS,
				OrderStatus.getCoverLegOrderDisplayStatus(sMainOrderStatus, sSLOrderStatus, sSquareoffStatus));

		OrderStatus.getCoverBuyOrSellMoreFlags(sMainOrderStatus, sSLOrderStatus, sSquareoffStatus, sBuyOrSell, order);
		...
		coverOrdObj.put(OrderConstants.CANCELLED_ORDER_DETAILS, cancelledLegOrderArr);
		return coverOrdObj;
	}
	
	private static JSONObject getCoverLegOrderDetails(JSONArray coverOrders, SymbolRow order)
            throws JSONException, Exception {
...
//                    coverOrderArr.put(slOrderObj);
                ...
                order.put(OrderConstants.REASON, OrderStatus.getErrorMsg(sSLOrderStatus, orderRow.getError()));
            }else {
                sSquareoffStatus = OrderStatus.getStatus(orderRow.getOrdStat());
                if (sSquareoffStatus.equals(OrderConstants.EXECUTED)) {
                    legOrderObj.put(OrderConstants.EXECUTED_ORDER, OrderConstants.DISP_SQUAREOFF_ORDER);
        ...
        order.put(OrderConstants.IS_CANCELLABLE,
                Boolean.toString(OrderStatus.isCoverOrderCancellable(sMainOrderStatus, sSLOrderStatus, false)));
        order.put(OrderConstants.EXIT_OPTION,
                Boolean.toString(false));
        
        //Repeat Order
        order.put(OrderConstants.IS_REPEAT_ORDER, "false" );
        ...
        legOrderObj.put(OrderConstants.DISP_STATUS,
                OrderStatus.getCoverLegOrderDisplayStatus(sMainOrderStatus, sSLOrderStatus, sSquareoffStatus));
        OrderStatus.getBuyOrSellMoreFlags(OrderConstants.CANCELLED, sBuyOrSell, order);     
	...
	private static JSONObject getCoverSquareOffOrderDetails(JSONArray coverOrders, SymbolRow order)
            throws JSONException, Exception {
...
//                    coverOrderArr.put(slOrderObj);
        ...
        order.put(OrderConstants.IS_MODIFIABLE, Boolean.toString(false));
        order.put(OrderConstants.IS_CANCELLABLE, Boolean.toString(false));
        order.put(OrderConstants.EXIT_OPTION, Boolean.toString(false));
	...
	private static JSONObject getBracketSLOrderDetails(JSONArray brackerOrders, SymbolRow order)
			throws JSONException, Exception {
		...
		order.put(OrderConstants.IS_MODIFIABLE, Boolean
				.toString(OrderStatus.isBracketOrderModifiable_101(sMainOrderStatus, sSLOrderStatus, sProfitOrderStatus, OrderConstants.IS_BRACKET_STOPLOSS_ORDER)));
		order.put(OrderConstants.IS_CANCELLABLE, Boolean
				.toString(OrderStatus.isBracketOrderModifyOrCancellable(sSLOrderStatus, sMainOrderStatus, sProfitOrderStatus,false)));
		order.put(OrderConstants.EXIT_OPTION, false);//Boolean.toString(OrderStatus.getBracketOrderExitOption_101(sMainOrderStatus, sSLOrderStatus, false)));
		//Repeat Order
        order.put(OrderConstants.IS_REPEAT_ORDER, "false" );
        //OrderAction for leg orders
        order.put(OrderConstants.BRACKET_ORDER_ACTION, orderAction);
	...
	/*** Parse group of bracket orders into single order ***/
	private static JSONObject getBracketProfitOrderDetails(JSONArray brackerOrders, SymbolRow order)
			throws JSONException, Exception {
		...
		for (int i = 0; i < brackerOrders.length(); i++) {
			...
			if (orderRow.getLegIndicator().equals(OrderConstants.MAIN_LEG_INDICATOR_9)) {
				...
				order.put(OrderConstants.REASON, OrderStatus.getErrorMsg(sProfitOrderStatus, orderRow.getError()));
			} else // If leg indicator is not any of the above case, then it is square off order
			{
				sSquareoffStatus = OrderStatus.getStatus(orderRow.getOrdStat());
				if (sSquareoffStatus.equals(OrderConstants.EXECUTED)) {
		...
		order.put(OrderConstants.IS_MODIFIABLE, Boolean
				.toString(OrderStatus.isBracketOrderModifiable_101(sMainOrderStatus, sSLOrderStatus, sProfitOrderStatus, OrderConstants.IS_BRACKET_PROFIT_ORDER)));
		order.put(OrderConstants.IS_CANCELLABLE, Boolean
				.toString(OrderStatus.isBracketOrderModifyOrCancellable(sProfitOrderStatus, sMainOrderStatus, sSLOrderStatus,false)));
		order.put(OrderConstants.EXIT_OPTION, false);//Boolean.toString(OrderStatus.getBracketOrderExitOption_101(sMainOrderStatus, sProfitOrderStatus, false)));
		//Repeat Order
        order.put(OrderConstants.IS_REPEAT_ORDER, "false" );
        //OrderAction for leg orders
        order.put(OrderConstants.BRACKET_ORDER_ACTION, orderAction);
	...
	private static JSONObject getBracketSquareOffOrderDetails(JSONArray brackerOrders, SymbolRow order)
            throws JSONException, Exception {
        ...
        for (int i = 0; i < brackerOrders.length(); i++) {
            ...
            } else // If leg indicator is not any of the above case, then it is square off order
            {
                getOrderDetails(orderRow, order);
                sSquareoffStatus = OrderStatus.getStatus(orderRow.getOrdStat());
                order.put(OrderConstants.REASON, OrderStatus.getErrorMsg(sSquareoffStatus, orderRow.getError()));
                order.put(OrderConstants.STATUS, sSquareoffStatus);
                if (sSquareoffStatus.equals(OrderConstants.EXECUTED)) {
                    legOrderObj.put(OrderConstants.EXECUTED_ORDER, OrderConstants.DISP_SQUAREOFF_ORDER);
                    legOrderObj.put(OrderConstants.ORDER_QTY, orderRow.getQty());
                    ...
                    legOrderObj.put(OrderConstants.PRICE, orderRow.getPrc());
                    order.put(OrderConstants.DISP_STATUS, OrderConstants.DISP_EXECUTED);
                }else if(sSquareoffStatus.equals(OrderConstants.PENDING) ){
                    order.put(OrderConstants.DISP_STATUS, OrderConstants.DISP_PENDING);
                }
        ...
        order.put(OrderConstants.IS_MODIFIABLE, Boolean
                .toString(OrderStatus.isBracketOrderModifiable_101(sMainOrderStatus, sSLOrderStatus, sProfitOrderStatus, OrderConstants.IS_BRACKET_SQUAREOFF_ORDER)));
        order.put(OrderConstants.IS_CANCELLABLE, Boolean
                .toString(false));//OrderStatus.isBracketOrderModifyOrCancellable(sSquareoffStatus, sSLOrderStatus, sProfitOrderStatus)));
        order.put(OrderConstants.EXIT_OPTION, Boolean
                .toString(false));//OrderStatus.getBracketOrderExitOption_101(sMainOrderStatus, sSquareoffStatus, false)));
        //Repeat Order
        order.put(OrderConstants.IS_REPEAT_ORDER, "false" );
        //OrderAction for leg orders
        order.put(OrderConstants.BRACKET_ORDER_ACTION, orderAction);
	...
	private static void getGTDOrdersCount(GTDOrderBook gtdOrderBook, JSONObject orderBookObj) {

        if (gtdOrderBook.getGTDExecutedSummary().size() > 0)
            orderBookObj.put(DeviceConstants.EXECUTED_COUNT, String.valueOf(gtdOrderBook.getGTDExecutedSummary().size()));

        if(gtdOrderBook.getGTDPendingSummary().size() > 0)
            orderBookObj.put(OrderConstants.GTC_COUNT, String.valueOf(gtdOrderBook.getGTDPendingSummary().size()));
        
        if(gtdOrderBook.getGTDCancelledSummary().size() > 0)
            orderBookObj.put(DeviceConstants.CANCELLED_COUNT, String.valueOf(gtdOrderBook.getGTDCancelledSummary().size()));

	}
	
	private static String getRecoID(String recoID) {
		recoID = recoID.trim();
		String id[] = recoID.split(" ");
		if(id.length > 3)
			recoID =  id[0]+id[1]+id[2];
		else if(id.length > 2)
			recoID =  id[0]+id[1];
		else if(id.length > 1)
			recoID = id[0];
