...
public class OrderBook_102 {
	...
	public static JSONObject getOrderBook(List<GetOrderBookObjectRow> orderRows, List<GetTradeBookObjectRow> tradeRows,
		...
		for (int i = 0; i < orderRows.size(); i++) {
			try {
				...
				}else if ( !orderRow.getRecoId().isEmpty() && orderRow.getProdType().equalsIgnoreCase(FTConstants.MARGIN_PLUS) ) { // Cover order
					/*** If it is main leg, get normal order details also ***/
					if ((orderRow.getLegIndicator().equals("0")) ? orderRow.getOrderType().equalsIgnoreCase(FTConstants.RL_MKT) : orderRow.getLegIndicator().equals(FTConstants.ONE_FLAG)) {
						SymbolRow order = getNormalOrderDetails(orderRow, linkedsetSymbolToken, sOrderStatus, orderBookObj);
						if (order.length() > 1)
							orderCollection.add(order);
		...
		/*** to get details of all the Cover orders ***/
		mapCoverOrdersParsed = getCoverOrderDetails(mapCoverOrders, linkedsetSymbolToken, sOrderStatus);
		/***
		 * to replace main order leg in normal order collections from the parsed bracket and cover
		 * order map
		 ***/
		...
		while (iterator.hasNext()) {
			...
			else if(mapCoverOrdersParsed.containsKey((order.has(OrderConstants.RECO_ID))? order.getString(OrderConstants.RECO_ID):null)) {
	...
	private static SymbolRow getNormalOrderDetails(GetOrderBookObjectRow orderRow,
			LinkedHashSet<String> linkedsetSymbolToken, String sOrderStatus, JSONObject orderBookObj) throws Exception {
		...
		if(orderRow.getIsGTD().equals("1"))//To Identify GTD Order
		...
		if(sStatus.equalsIgnoreCase(OrderConstants.PENDING)) {
			orderBookObj.put(DeviceConstants.PENDING_COUNT, orderBookObj.has(DeviceConstants.PENDING_COUNT) ? String.valueOf(Integer.parseInt(orderBookObj.getString(DeviceConstants.PENDING_COUNT))+1) : "1");
		}else if(sStatus.equalsIgnoreCase(OrderConstants.CANCELLED)) {
			orderBookObj.put(DeviceConstants.CANCELLED_COUNT, orderBookObj.has(DeviceConstants.CANCELLED_COUNT) ? String.valueOf(Integer.parseInt(orderBookObj.getString(DeviceConstants.CANCELLED_COUNT))+1) : "1");
		}else if(sStatus.equalsIgnoreCase(OrderConstants.EXECUTED)) {
			orderBookObj.put(DeviceConstants.EXECUTED_COUNT, orderBookObj.has(DeviceConstants.EXECUTED_COUNT) ? String.valueOf(Integer.parseInt(orderBookObj.getString(DeviceConstants.EXECUTED_COUNT))+1) : "1");
		}
	...
	private static JSONObject getProfitOrderDetails(GetOrderBookObjectRow orderRow, boolean isMainOrderPending, SymbolRow order)
		...
		targetOrderObj.put(OrderConstants.ORDER_VALUE, getOrderTradeValue(PriceFormat.formatPrice(
				String.valueOf((new BigDecimal(orderRow.getProfitOrdPrc()).multiply(new BigDecimal(orderRow.getQty())))),
				Integer.parseInt(OrderConstants.PRECISION_2), false), SymbolMap.getSymbolRow(order.getSymbolToken())));
	...
	private static JSONObject getSLOrderDetails(GetOrderBookObjectRow orderRow, boolean isMainOrderPending, SymbolRow order)
		...
		slOrderObj.put(OrderConstants.ORDER_VALUE, getOrderTradeValue(PriceFormat.formatPrice(
				String.valueOf((new BigDecimal(orderRow.getSlOrdPrc()).multiply(new BigDecimal(orderRow.getQty())))),
				Integer.parseInt(OrderConstants.PRECISION_2), false), SymbolMap.getSymbolRow(order.getSymbolToken())));
	...
	private static JSONObject getMainOrderDetails(GetOrderBookObjectRow orderRow, SymbolRow order) throws JSONException, Exception {
		...
		mainOrderObj.put(OrderConstants.ORDER_VALUE, getOrderTradeValue(PriceFormat.formatPrice(
				String.valueOf((new BigDecimal(orderRow.getPrc()).multiply(new BigDecimal(orderRow.getQty())))),
				Integer.parseInt(OrderConstants.PRECISION_2), false), SymbolMap.getSymbolRow(order.getSymbolToken())));
	...
	private static Map<String, SymbolRow> getCoverOrderDetails(Map<String, JSONArray> mapCoverOrders,
		...
		for (Map.Entry<String, JSONArray> entry : mapCoverOrders.entrySet()) {
			...
			String sTokenMktSegID = sScripCode + "_" + sMktSegID;
			order.extend(SymbolMap.getSymbolRow(sScripCode, sMktSegID).getMinimisedSymbolRow());
			order.put(FTConstants.CO_DETAILS, getCoverOrderDetails(entry.getValue(), order));
			order.put(OrderConstants.IS_BRACKET_ORDER, "false");
	...
	private static JSONObject getCoverOrderDetails(JSONArray coverOrders, SymbolRow order)
			throws JSONException, Exception {
		...
		for (int i = 0; i < coverOrders.length(); i++) {
			...
			if ((orderRow.getLegIndicator().equals("0")) ? orderRow.getOrderType().equalsIgnoreCase(FTConstants.RL_MKT) : orderRow.getLegIndicator().equals("1")) {
				...
				modifyOrderDetailsObj.put(OrderConstants.MAIN_ORDER_MODIFY, getModifyOrderDetails(orderRow, false).put(FTConstants.CLIENT_ORD_NO, orderRow.getClientOrdNo()));
				modifyOrderDetailsObj.put(OrderConstants.BRACKET_ORDER_ID, orderRow.getBracketOrdId());
				modifyOrderDetailsObj.put(OrderConstants.PRICE, orderRow.getPrc());
				modifyOrderDetailsObj.put(OrderConstants.ORDER_QTY, orderRow.getQty());
				modifyOrderDetailsObj.put(OrderConstants.PENDING_QTY, orderRow.getQty());

			} else if ((orderRow.getLegIndicator().equals("0")) ? orderRow.getOrderType().equalsIgnoreCase(FTConstants.RL) : orderRow.getLegIndicator().equals("2")) {
				/***
				 * To fetch Stop loss order details, flag is to identify main order is pending
				 * or not
				 ***/
				...
				if (sSLOrderStatus.equals(OrderConstants.PENDING)) {
					modifyOrderDetailsObj.put(OrderConstants.IS_SL_ORDER_MODIFIABLE, "true");
					modifyOrderDetailsObj.put(OrderConstants.SL_ORDER_MODIFY, getModifyOrderDetails(orderRow, false).put(FTConstants.CLIENT_ORD_NO, orderRow.getClientOrdNo()));
				}
			} 
		...
		order.put(OrderConstants.IS_MODIFIABLE, 
				Boolean.toString(OrderStatus.isCoverOrderModifiable(sMainOrderStatus, sSLOrderStatus)));
		order.put(OrderConstants.IS_CANCELLABLE, 
				Boolean.toString(OrderStatus.isCoverOrderCancellable(sMainOrderStatus, sSLOrderStatus,true)));
		order.put(OrderConstants.EXIT_OPTION, 
				Boolean.toString(OrderStatus.getCoverOrderExitOption(sMainOrderStatus, sSLOrderStatus)));
		
		//Temporary fix
		if (OrderStatus.getCoverOrderConvertOption(sMainOrderStatus, sSLOrderStatus)) {
